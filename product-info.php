...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($product_name_input_err) && empty($category_err) && empty($rpoint_err) && empty($qoh_err) && empty($original_price_err) && empty($selling_price_err)){
        ...
        $sql = "UPDATE product SET name = ?, unit_sale_price = ?, unit_cost_price = ?, quantity_unit_type = ?, quantity_on_hand = ?, reorder_point = ?, category_id = ? WHERE product.id = ?";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sdddddii", $param_product_name_input, $param_unit_sale_price, $param_unit_cost_price, $param_quantity_unit_type, $param_quantity_on_hand, $param_reorder_point, $param_category_id, $param_product_id);
        ...
        <table class="table table-hover table-responsive results">
            ...
            <tbody id="dynamic-product-table">
                ...
                if(isset($_POST["id"]) && !empty($_POST["id"])){
                    ...
                    $sql = "SELECT * FROM stock_in WHERE product_id=" . $prod_id;
                    if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                            while($row = mysqli_fetch_array($result)){
                                echo "<tr scope='row'>";
